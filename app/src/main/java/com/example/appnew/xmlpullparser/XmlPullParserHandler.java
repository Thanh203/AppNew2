...
public class XmlPullParserHandler {
    ...
    public List<TinTuc> Pasers(InputStream in) throws XmlPullParserException , IOException {
        try {
    ...
    private List<TinTuc> readRss(XmlPullParser parser) throws XmlPullParserException , IOException {
        Log.i("-----", "read rss");
    ...
    private TinTuc readItem(XmlPullParser parser) throws XmlPullParserException , IOException {
        Log.i("-----", "read Item");
        ...
        return new TinTuc(title, link, pubDate, img);
    }
    ...
    private String getContentFromText(String a){

        if (a.contains("</br>")){
            int posImg = a.indexOf("br");// lấy vị trí chuỗi "img" trong chỗi
            int posDong = a.indexOf("]]>");
            a = a.substring(posImg, posDong);
            a = a.substring(a.indexOf(">") + 1, a.length() - 3);
        }
    ...
    private String readContent(XmlPullParser parser)throws XmlPullParserException , IOException{
        ...
        while (parser.next() != XmlPullParser.END_TAG){
            String name = parser.getText();
            content = getContentFromText(name);
            Log.i("-----", "read description Content: "+content);
        }
        return content;
    }
